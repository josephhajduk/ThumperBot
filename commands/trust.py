...
class Trust(BotCommand):
    def __init__(self, telegram_id, bot):
        super(Trust, self).__init__(telegram_id, bot)
        self.current_handler = self.initial_handler

    key = "/trust"
    description = "Raises a user's auth level to 2"
    auth_level = 4
    ...
    def handle_character(self, msg, chat_handler):
        if (yield from assert_text(msg,chat_handler)):
            ...
            if len(Character.select().where(Character.name == char_name)) > 0:
                ...
                user.auth_level = max(user.auth_level, 2)
                user.save()
                self.finished()
                yield from chat_handler.sender.sendMessage(_s["msg_usertrusted"])
            else:
